...
namespace ElBasset.Controllers.Student
{
    ...
    public class StudentController : Controller
    {
        ...
        public StudentController(IUnitOfWork unit, SignInManager<ApplicationUser> sign, UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> RoleManager)
        {
            this._unit = unit;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            await Data();
            var AllUser = new List<ApplicationUser>();
            return View(AllUser);
        }
        [HttpGet]
        ...
        {
            List<ApplicationUser> AllUser = await _userManager.Users.ToListAsync();
            if (DepartmentId != null)
                AllUser = AllUser.Where(s => s.DepartmentId == DepartmentId).ToList();
            await Data();
            return View("Index", AllUser);

        }
        public async Task<IActionResult> FilterStudent(int? DepartmentId, string Name)
        {
            List<ApplicationUser> AllUser = await _userManager.Users.ToListAsync();
            if (DepartmentId != null)
                AllUser = AllUser.Where(s => s.DepartmentId == DepartmentId).ToList();
            if (!string.IsNullOrEmpty(Name))
                AllUser = AllUser.Where(s => s.DepartmentId == DepartmentId
                           && s.Email.ToLower().Contains(Name.ToLower())).ToList();
            await Data();
            return View("Index", AllUser);
